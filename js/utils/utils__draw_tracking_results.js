...
export const drawFaceDetectionResults = (brfv5Manager, brfv5Config, canvas) => {
  ...
  drawRect( ctx, brfv5Config.faceTrackingConfig.regionOfInterest, colorSecondary, 2.0*10)

//  drawRect( ctx, brfv5Config.faceDetectionConfig.regionOfInterest, colorSecondary, 2.0*10)
//  drawRects(ctx, brfv5Manager.getDetectedRects(), colorPrimary, 1.0*10)
//  drawRects(ctx, brfv5Manager.getMergedRects(), colorSecondary, 3.0*10)
}
  ...
  if(brfv5Config.enableFaceTracking) {
    ...
    for(let i = 0; i < faces.length; i++) {
      ...
      if(face.state === brfv5.BRFv5State.FACE_TRACKING) {
        ...
        drawRect(ctx, face.bounds, colorSecondary, 1.0*10)
